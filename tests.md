<!-- Generated by jsonstor-docs/facility/test-cli.js --publish-report. Edits here are erased by the next run. -->

# @liquicode/jsonstor

# Test Results

Every test in this family lives in [`@liquicode/jsonstor-docs`](https://github.com/liquicode/jsonstor-docs)
  and is run from there. This page is that run, filtered to this package.

## Unit Tests

| Suite                                                                                                           | Passing | Failing | Skipped |  Duration |
|-----------------------------------------------------------------------------------------------------------------|--------:|--------:|--------:|----------:|
| 001) Adapter Registry Tests A package carrying a single adapter                                                 |       3 |       0 |       0 |       1ms |
| 001) Adapter Registry Tests A package carrying a family of adapters                                             |       5 |       0 |       0 |       1ms |
| 001) Adapter Registry Tests A package carrying aliases                                                          |      12 |       0 |       0 |       2ms |
| 002) Dialect Boundary Tests The registry knows a family                                                         |       2 |       0 |       0 |       1ms |
| 002) Dialect Boundary Tests A server inside its prime range                                                     |       3 |       0 |       0 |       1ms |
| 002) Dialect Boundary Tests A server which crossed a boundary                                                   |       4 |       0 |       0 |       0ms |
| 002) Dialect Boundary Tests A server newer than the dialect in force                                            |       3 |       0 |       0 |       1ms |
| 002) Dialect Boundary Tests A server past everything measured                                                   |       3 |       0 |       0 |       0ms |
| 002) Dialect Boundary Tests Nothing to check                                                                    |       2 |       0 |       0 |       0ms |
| 003) Dialect Profile Tests Every profile in the run is one this file records                                    |       2 |       0 |       0 |       0ms |
| 003) Dialect Profile Tests Each profile renders with what was measured jsonstor-mysql-v5.7                      |       3 |       0 |       1 |       0ms |
| 003) Dialect Profile Tests Each profile renders with what was measured jsonstor-postgres-v10.21                 |       0 |       0 |       4 |       0ms |
| 003) Dialect Profile Tests Each profile renders with what was measured jsonstor-oracle-v18.0                    |       0 |       0 |       4 |       0ms |
| 003) Dialect Profile Tests Each profile renders with what was measured jsonstor-oracle-v23.26                   |       0 |       0 |       4 |       0ms |
| 003) Dialect Profile Tests Each profile renders with what was measured jsonstor-mssql-v14.0                     |       0 |       0 |       4 |       0ms |
| 003) Dialect Profile Tests Each profile renders with what was measured jsonstor-sqlite                          |       3 |       0 |       1 |       0ms |
| 003) Dialect Profile Tests Each profile renders with what was measured jsonstor-duckdb                          |       3 |       0 |       1 |       0ms |
| 003) Dialect Profile Tests Each Mango profile declares what was measured                                        |       1 |       0 |       0 |       0ms |
| 003) Dialect Profile Tests Each Mango profile declares what was measured jsonstor-mongodb-v4.4                  |       2 |       0 |       1 |       0ms |
| 003) Dialect Profile Tests Each Mango profile declares what was measured jsonstor-couchdb-v2.3                  |       2 |       0 |       1 |       0ms |
| 003) Dialect Profile Tests What a version boundary changes                                                      |       4 |       0 |       0 |       0ms |
| 004) Unreachable Storage Tests A storage pointed at something which cannot answer                               |       9 |       0 |       0 |     1.34s |
| 005) Payload Containment Tests A shape which was measured to agree is rendered                                  |       8 |       0 |       0 |       2ms |
| 005) Payload Containment Tests A shape which was measured to lose a row is never rendered                       |       9 |       0 |       0 |       1ms |
| 005) Payload Containment Tests Nothing outside the measured set is rendered at all                              |       5 |       0 |       0 |       3ms |
| 005) Payload Containment Tests The dialect decides whether any of this happens at all                           |       6 |       0 |       0 |       0ms |
| 006) Server Identity Tests Every server-backed engine reports the server it reached                             |       1 |       0 |       0 |       0ms |
| 006) Server Identity Tests Every server-backed engine reports the server it reached jsonstor-mongodb-v8.3       |       4 |       0 |       0 |       0ms |
| 006) Server Identity Tests Every server-backed engine reports the server it reached jsonstor-mysql-v5.7         |       4 |       0 |       0 |       0ms |
| 006) Server Identity Tests Every server-backed engine reports the server it reached jsonstor-redis-v6.2         |       4 |       0 |       0 |       0ms |
| 006) Server Identity Tests Every server-backed engine reports the server it reached jsonstor-couchdb-v2.3       |       4 |       0 |       0 |       0ms |
| 006) Server Identity Tests Every server-backed engine reports the server it reached jsonstor-elasticsearch-v9.5 |       4 |       0 |       0 |       0ms |
| 006) Server Identity Tests Every server-backed engine reports the server it reached jsonstor-opensearch-v3.8    |       4 |       0 |       0 |       0ms |
| 006) Server Identity Tests Every server-backed engine reports the server it reached jsonstor-couchbase-v8.0     |       4 |       0 |       0 |       0ms |
| 006) Server Identity Tests Redis and Valkey are told apart                                                      |       2 |       0 |       0 |       1ms |
| 007) Transport Security Tests The inventory                                                                     |       2 |       0 |       0 |       0ms |
| 007) Transport Security Tests The inventory jsonstor-mongodb                                                    |       4 |       0 |       0 |       0ms |
| 007) Transport Security Tests The inventory jsonstor-mysql                                                      |       4 |       0 |       0 |       0ms |
| 007) Transport Security Tests The inventory jsonstor-postgres                                                   |       4 |       0 |       0 |       0ms |
| 007) Transport Security Tests The inventory jsonstor-oracle                                                     |       4 |       0 |       0 |       0ms |
| 007) Transport Security Tests The inventory jsonstor-mssql                                                      |       4 |       0 |       0 |       0ms |
| 007) Transport Security Tests The inventory jsonstor-redis                                                      |       4 |       0 |       0 |       0ms |
| 007) Transport Security Tests The inventory jsonstor-couchdb                                                    |       4 |       0 |       0 |       0ms |
| 007) Transport Security Tests The inventory jsonstor-elasticsearch                                              |       4 |       0 |       0 |       0ms |
| 007) Transport Security Tests The inventory jsonstor-dynamodb                                                   |       4 |       0 |       0 |       0ms |
| 007) Transport Security Tests The inventory jsonstor-couchbase                                                  |       4 |       0 |       0 |       0ms |
| 007) Transport Security Tests Against a server which requires TLS                                               |       0 |       0 |       3 |       0ms |
| 008) Primary Key Tests What the settings declared                                                               |       6 |       0 |       0 |       0ms |
| 008) Primary Key Tests The value a key is filed under                                                           |       6 |       0 |       0 |       0ms |
| 008) Primary Key Tests What the index will answer a criteria with                                               |       7 |       0 |       0 |       0ms |
| 008) Primary Key Tests The index structure                                                                      |       5 |       0 |       0 |       0ms |
| 008) Primary Key Tests An index is a pushdown                                                                   |       5 |       0 |       0 |       4ms |
| 008) Primary Key Tests NewUniqueID                                                                              |       8 |       0 |       0 |      10ms |
| 009) Paging Tests Normalize                                                                                     |       4 |       0 |       0 |       1ms |
| 009) Paging Tests Apply                                                                                         |       2 |       0 |       0 |       0ms |
| 009) Paging Tests The interface                                                                                 |       1 |       0 |       0 |       0ms |
| 010) SqlExpression Tests                                                                                        |      46 |       0 |       0 |       2ms |
| 011) SqlExpression Coverage Tests                                                                               |       9 |       0 |       0 |       3ms |
| 012) MangoExpression Tests Registration                                                                         |       3 |       0 |       0 |       0ms |
| 012) MangoExpression Tests Absorption                                                                           |       8 |       0 |       0 |       2ms |
| 012) MangoExpression Tests Pruning                                                                              |       9 |       0 |       0 |       0ms |
| 012) MangoExpression Tests Narrowing the vocabulary                                                             |       4 |       0 |       0 |       0ms |
| 012) MangoExpression Tests The invariant                                                                        |       3 |       0 |       0 |      82ms |
| 012) MangoExpression Tests The absence test repair                                                              |       7 |       0 |       0 |       2ms |
| 012) MangoExpression Tests Rendering a broadening operator                                                      |       5 |       0 |       0 |       1ms |
| 012) MangoExpression Tests The array element repair                                                             |       8 |       0 |       0 |       0ms |
| 012) MangoExpression Tests Absence and negation on a path                                                       |       7 |       0 |       0 |       0ms |
| 012) MangoExpression Tests A schema is not a criteria                                                           |       4 |       0 |       0 |       1ms |
| 012) MangoExpression Tests The operand, not the name alone                                                      |       8 |       0 |       0 |       1ms |
| 013) DynamoExpression Tests The triple                                                                          |       4 |       0 |       0 |       1ms |
| 013) DynamoExpression Tests Equality, which is a pair because jsongin matches array elements                    |       4 |       0 |       0 |       0ms |
| 013) DynamoExpression Tests Negation, and the guard the emulator says is unnecessary                            |       4 |       0 |       0 |       1ms |
| 013) DynamoExpression Tests Ordering, which needs a scalar promise                                              |       3 |       0 |       0 |       0ms |
| 013) DynamoExpression Tests The operators whose rendering carries a guard                                       |       7 |       0 |       0 |       1ms |
| 013) DynamoExpression Tests What is not rendered, and each one is a measurement                                 |       7 |       0 |       0 |       0ms |
| 013) DynamoExpression Tests A list of one renders no redundant parentheses                                      |       3 |       0 |       0 |       0ms |
| 013) DynamoExpression Tests The logical operators                                                               |       3 |       0 |       0 |       0ms |
| 013) DynamoExpression Tests The fidelity table                                                                  |       2 |       0 |       0 |       0ms |
| 013) DynamoExpression Tests The invariant, as a property rather than an example                                 |       4 |       0 |       0 |      60ms |
| 014) N1qlExpression Tests Field references — why this is not a SqlExpression dialect                            |       7 |       0 |       0 |       2ms |
| 014) N1qlExpression Tests Array in the middle of a path — the repair MangoExpression measured first             |       7 |       0 |       0 |       1ms |
| 014) N1qlExpression Tests $all against a field which is not an array                                            |       3 |       0 |       0 |       0ms |
| 014) N1qlExpression Tests The four spellings SqlExpression was refused for                                      |       4 |       0 |       0 |       1ms |
| 014) N1qlExpression Tests Negation — the expression is guarded, never the field                                 |       4 |       0 |       0 |       0ms |
| 014) N1qlExpression Tests Type guards — jsongin refuses a cross type ordering and N1QL does not                 |       2 |       0 |       0 |       0ms |
| 014) N1qlExpression Tests Candidate resolution — an array field matches on its elements                         |      11 |       0 |       0 |       0ms |
| 014) N1qlExpression Tests Absent and null — the pair no other target here can separate                          |       9 |       0 |       0 |       1ms |
| 014) N1qlExpression Tests Regular expressions — RE2 is not Javascript                                           |       4 |       0 |       0 |       0ms |
| 014) N1qlExpression Tests What is deliberately not rendered                                                     |       4 |       0 |       0 |       0ms |
| 014) N1qlExpression Tests The logical operators and their asymmetry                                             |       3 |       0 |       0 |       0ms |
| 014) N1qlExpression Tests The residual contract                                                                 |       7 |       0 |       0 |       1ms |
| 014) N1qlExpression Tests The fidelity table                                                                    |       5 |       0 |       0 |       1ms |
| 014) N1qlExpression Tests AllowedFields — the default is the opposite of SqlExpression's, deliberately          |       2 |       0 |       0 |       0ms |
| 015) Excel Workbook Tests                                                                                       |       3 |       0 |       0 |     117ms |
| 016) Memory Documents Tests A storage which was given documents                                                 |       8 |       0 |       0 |       2ms |
| 017) Undo Tests                                                                                                 |       9 |       0 |       0 |       4ms |
| 020) OpLog Filter Tests                                                                                         |       9 |       0 |       0 |      87ms |
| 030) Browser Bundle Tests                                                                                       |      14 |       0 |       0 |       2ms |
| 031) Browser Storage Tests                                                                                      |      29 |       0 |       0 |       1ms |
| **total**                                                                                                       | **517** |   **0** |  **24** | **1.74s** |

## Engine Tests

| Engine | A | B | C | D | E | F | G | H | I | M | N | Z | Total |
|--------|----:|----:|----:|----:|----:|----:|----:|----:|----:|----:|----:|----:|------:|

Each cell is ***passing/failing (duration)***.

| Column | Category                   |
|--------|----------------------------|
| A      | CRUD Tests                 |
| B      | Rainbow Query Tests        |
| C      | UserInfo Permissions Tests |
| D      | Engine Contract Tests      |
| E      | Extension Operator Tests   |
| F      | Pushdown Tests             |
| G      | Mango Acceptance Tests     |
| H      | Engine Parity Tests        |
| I      | Undo Tests                 |
| M      | MongoDB Tutorial           |
| N      | MongoDB Reference          |
| Z      | Ad-Hoc Tests               |

## Failures

***No failures.***
